<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  class Datatables
  {
    ...
    * @return mixed
    */
    public function select($columns, $backtick_protect = TRUE,$as_protect = TRUE)
    {
      if($as_protect)
      {
    ...
    * @return mixed
    */
    public function order_by($col,$sort)
    {
      
      $this->ci->db->order_by($col,$sort);
      return $this;
    }
  ...
  /**
        * BY HIM
        * To run a custom query instead on using codeignitor's active record;
        *
        * @param string $sql
        */
        public function custom_query($sql,$grp_by_part)
        {
          ...
          $original_sql = $sql.$grp_by_part ;
            
          $columns = "title, email, coalesce(view_count,0) as views, coalesce(cheers_count,0) as cheers, coalesce(bonus_count,0) as bonus, t.created_at, t.id, uploaded_via_admin, t.processed, clip_url";
          foreach($this->explode(',', $columns) as $val)
          {
            $column = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$2', $val));
            $this->columns[] =  $column;
      ...
      for($i = 0; $i < intval($this->ci->input->post('iColumns')); $i++)
      {
        if(isset($_POST['sSearch_' . $i]) && $this->ci->input->post('sSearch_' . $i) != '' && in_array($mColArray[$i], $columns))
        {
          ...
          {
            if(preg_match("/(<=|>=|=|<|>)(\s*)(.+)/i", trim($val), $matches))
           $sql.= ' AND '.$this->select[$mColArray[$i]].' '.$matches[1]. ' = '.$matches[3];
             //$this->ci->db->where($this->select[$mColArray[$i]].' '.$matches[1], $matches[3]);
            elseif(!empty($sRangeSeparator) && preg_match("/(.*)$sRangeSeparator(.*)/i", trim($val), $matches))
            {
              $rangeQuery = '';
              ...
              $sql.= ' AND '.$this->select[$mColArray[$i]] . ' LIKE '.'%' . $val . '%';
              //$this->ci->db->where($this->select[$mColArray[$i]] . ' LIKE', '%' . $val . '%');
          }
        }
      }
      ...
      for($i = 0; $i < intval($this->ci->input->post('iSortingCols')); $i++)
                if(isset($mColArray[intval($this->ci->input->post('iSortCol_' . $i))]) && in_array($mColArray[intval($this->ci->input->post('iSortCol_' . $i))], $columns) && $this->ci->input->post('bSortable_'.intval($this->ci->input->post('iSortCol_' . $i))) == 'true')
                 $sql.= " ORDER BY ".$mColArray[intval($this->ci->input->post('iSortCol_' . $i))]. "  " .$this->ci->input->post('sSortDir_' . $i); 
            
       
       /** PAGING**/    
        $iStart = ($this->ci->input->post('iDisplayStart')) ? $this->ci->input->post('iDisplayStart') : 0 ;
        $iLength = $this->ci->input->post('iDisplayLength');

        if($iLength != '' && $iLength != '-1')
        {
           $sql.= " OFFSET  $iStart LIMIT $iLength";  
        }
        ...
        $query  =   $this->ci->db->query($sql);      
                      ...
                      if($output == 'json')
                      {
                        $query_for_total = $this->ci->db->query($original_sql);
                        $query_for_filtlred_total = $this->ci->db->query($query_after_filtering);
                        
                        $iTotal = $query_for_total->num_rows();
                        $iFilteredTotal = $query_for_filtlred_total->num_rows();
                      }
